...
public class VerRecepcionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ver_recepcion);
        ...
        String txt = "";
        db.openDB();
        Producto producto = db.getProducto(getIntent().getLongExtra(RecepcionesActivity.VALUE_IDPRODUCTO, 0));
        if (producto != null) {
        ...
        Almacen almacen = db.getAlmacen(getIntent().getLongExtra(RecepcionesActivity.VALUE_IDALMACEN, 0));
        if (almacen != null) {
        ...
        calendar.setTimeInMillis(getIntent().getLongExtra(RecepcionesActivity.VALUE_FECHA, 0));
        tvFecha.setText(Util.calendarToNumericString(calendar));

        adaptador.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spEstado.setAdapter(adaptador);

        tvCant.setText(getIntent().getIntExtra(RecepcionesActivity.VALUE_CANTIDAD, 0) + txt);
        spEstado.setEnabled(false);

        for (int i = 0; i < estadoList.size(); i++) {
            if (estadoList.get(i).getId() == getIntent().getLongExtra(RecepcionesActivity.VALUE_IDESTADO, 0)) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        inflater.inflate(R.menu.activity_ver_recepcion, menu);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.mnGuardar:
                ...
                result.putExtra(RecepcionesActivity.VALUE_ID, getIntent().getLongExtra(RecepcionesActivity.VALUE_ID, 0));
                result.putExtra(RecepcionesActivity.VALUE_IDPRODUCTO, getIntent().getLongExtra(RecepcionesActivity.VALUE_IDPRODUCTO, 0));
                Estado estado = (Estado) spEstado.getSelectedItem();
                result.putExtra(RecepcionesActivity.VALUE_IDESTADO, estado.getId());

                result.putExtra(RecepcionesActivity.VALUE_FECHA, getIntent().getLongExtra(RecepcionesActivity.VALUE_FECHA, 0));
                result.putExtra(RecepcionesActivity.VALUE_IDALMACEN, getIntent().getLongExtra(RecepcionesActivity.VALUE_IDALMACEN, 0));
                result.putExtra(RecepcionesActivity.VALUE_CANTIDAD, getIntent().getIntExtra(RecepcionesActivity.VALUE_CANTIDAD, 0));

                setResult(RecepcionesActivity.RESULT_CODE_MODIFICAR_RECEPCION, result);
                finish();
                return true;
            default:
